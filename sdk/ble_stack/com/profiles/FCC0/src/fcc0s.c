...
const struct attm_desc fcc0_att_db[FCC0S_IDX_NB] =
{
    ...
    [FCC0S_IDX_FCC2_LVL_VAL]   =   {ATT_USER_SERVER_CHAR_FCC2,PERM(WRITE_REQ, ENABLE)|PERM(WRITE_COMMAND, ENABLE)|PERM(RD, ENABLE), PERM(UUID_LEN, UUID_128)|PERM(RI, ENABLE)|ATT_UUID_128_LEN, FCC0_FCC2_DATA_LEN *sizeof(uint8_t)},
    ...
    [FCC0S_IDX_FCC1_LVL_VAL]   =   {ATT_USER_SERVER_CHAR_FCC1,PERM(WRITE_REQ, ENABLE)|PERM(WRITE_COMMAND, ENABLE)|PERM(RD, ENABLE), PERM(UUID_LEN, UUID_128)|PERM(RI, ENABLE)|ATT_UUID_128_LEN, FCC0_FCC1_DATA_LEN * sizeof(uint8_t)},
...
static uint8_t fcc0s_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct fcc0s_db_cfg* params)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        if(params->features == FCC0_FCC1_LVL_NTF_SUP)
        {
            ...
            uint16_t perm = PERM(NTF, ENABLE);//PERM(RD, ENABLE) | PERM(NTF, ENABLE);
