...
*/
#define USER_OAD_SERV_UUID_128  {0xA5, 0xA5, 0x00, 0x5B, 0x02, 0x00, 0x23, 0x9B, \
									  0xE1, 0x11, 0x02, 0xD1, 0x00, 0x11, 0x00, 0x00}
	
#define USER_OAD_CHAR_UUID_128  {0xA5, 0xA5, 0x00, 0x5B, 0x02, 0x00, 0x23, 0x9B, \
									  0xE1, 0x11, 0x02, 0xD1, 0x00, 0x11, 0x00, 0x00}



void attm_oad_serv_convert_to128(uint8_t *uuid128, const uint8_t *uuid, uint8_t uuid_len)
{
    uint8_t auc_128UUIDBase[ATT_UUID_128_LEN] = USER_OAD_SERV_UUID_128;
    uint8_t cursor = 0;
...
void attm_oad_char_convert_to128(uint8_t *uuid128, const uint8_t *uuid, uint8_t uuid_len)
{
    uint8_t auc_128UUIDBase[ATT_UUID_128_LEN] = USER_OAD_CHAR_UUID_128;
    uint8_t cursor = 0;
...
uint8_t attm_oad_util_svc_create_db128(uint16_t *shdl, uint16_t uuid, uint8_t *cfg_flag, uint8_t max_nb_att,uint8_t *att_tbl, ke_task_id_t const dest_id,const struct attm_desc *att_db, uint8_t svc_perm)
{
    uint8_t nb_att = 0;
    uint8_t i;
    ...
    // copy UUID
	attm_oad_serv_convert_to128(svc_desc->uuid,(uint8_t *)&uuid,ATT_UUID_16_LEN);
    ...
    for (i = 1; i<max_nb_att; i++)
    {
        ...
        if ((cfg_flag == NULL) || (((cfg_flag[i/8] >> (i%8)) & 1) == 1))
        {
			...
			{
				attm_oad_char_convert_to128(svc_desc->atts[nb_att].uuid,(uint8_t *)&(att_db[i].uuid),ATT_UUID_16_LEN);
			}
			else
			{
