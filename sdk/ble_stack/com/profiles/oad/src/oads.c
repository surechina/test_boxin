...
static uint8_t oads_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct oads_db_cfg* params)
{
	...
	status = attm_oad_util_svc_create_db128(start_hdl, ATT_USER_SERVER_SVC_OAD, (uint8_t *)&cfg_flag,
		   OADS_IDX_NB, NULL, env->task, &oads_att_db[0],
		  (sec_lvl & (PERM_MASK_SVC_DIS | PERM_MASK_SVC_AUTH | PERM_MASK_SVC_EKS))
		   | PERM(SVC_MI, DISABLE) );	
	#endif
...
uint32_t oad_updating_user_section_pro(void)
{
	...
	if(bsec.flag_write == 1)//oad_save_reciveData标志的，代表有东西需要写入flash
	{
		sec_ptr = &bsec;
    ...
    if( bsec.flag_write == 2 )//oad_updating_user_section_end标志的，代表写入最后一个包到flash
    {
        sec_ptr = &bsec;
        wr_addr = sec_ptr->update_offset + ( SEC_BACKUP_OAD_HEADER_FADDR );
        len = sec_ptr->data_cnt * 16;
	...
	if(bsec.flag_write >= 3)
	{
		bsec.flag_write++;
		cpu_idle_sleep();
	}
	if(bsec.flag_write >= 100)//轮训100次，确定没有下一个包了，重启
	{	
		for(int i=0; i<10; i++)
		{	
...
#endif
		ke_timer_set(APP_OAD_IN_PROGRESS_TIMER,TASK_APP,APP_OAD_IN_PROGRESS_PERIOD);

		uart_printf("##sure 11upgrade_in_progress = %d\n",upgrade_in_progress);
...
uint8_t oadImgBlockWrite( uint16_t connHandle, uint8_t *pValue )
{
	...
	uint8_t i = 0;
	//uint8_t pr_char[12]={0x41, 0x54, 0x2B, 0x44, 0x41, 0x54, 0x41, 0x3D, 0x01, 0x3D, 0x0D, 0x0A};
	uint8_t pr_char[9]="AT+LINK\r\n";
	...
	{
	//防止升级过程中，调用了appm_start_advertise();
	if(ke_timer_active(APP_PERIOD_TIMER, TASK_APP))
		ke_timer_clear(APP_PERIOD_TIMER, TASK_APP);
		upgrade_in_progress = 1;
#if ENABLE_UNUSED_SECTION_WRITE
    ...
    if (oadBlkNum == blkNum)
    {
		if(oadBlkNum != 0)
		{
			validBlockCnt++;
			oad_save_reciveData(pValue);
			for(i=0; i<oadBlkTot/500; i++)
			{
				if(oadBlkNum == i*500)
				{
					uart_send(pr_char,9);
					uart_printf("##sure oad=%d\n",upgrade_in_progress);
				}
			}
			
			/*if(validBlockCnt == (oadBlkTot - 2))
				upgrade_in_progress = 2;*/
			if(validBlockCnt == (oadBlkTot - 1))
			{
				UART_PRINTF("last block \r\n");
				//oadImgBlockReq(connHandle, oadBlkNum);
                oadImgBlockReq( connHandle, 0xFFFF );
