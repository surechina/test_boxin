    ...
    {				
		...
		*/
		...
		if(upgrade_in_progress == 1)
		{
			...
			//ke_msg_send_basic(APP_USER_TIMER_REQ,TASK_APP,TASK_APP);
			ke_timer_set(APP_PERIOD_TIMER,TASK_APP,APP_TIMER_PERIOD);
		}
    }
...
void appm_stop_advertising(void)
{
    if (ke_state_get(TASK_APP) == APPM_ADVERTISING)
    {
		...
		wdt_disable_flag = 1;
	 	uart_printf("#sure appm stop advertising\r\n");
    }
    // else ignore the request
}
